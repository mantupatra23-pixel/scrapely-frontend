...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("https://scrapely-backend.onrender.com/api/v1/auth/register", {
        method: "POST",
      ...
      // Successful registration -> Go to Sign In
      router.push("/signin");
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#1e293b] border border-slate-700/80 rounded-3xl p-8 shadow-2xl backdrop-blur-xl">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-white mt-6">Create Your Account</h1>
          <p className="text-xs text-slate-400 mt-1">Start extracting verified B2B leads instantly</p>
        </div>
